...
tags: [mcp, rest, api, tools, structured-query, list-connections, describe-table, query-templates]
next_actions:
...
Optionally, an admin may publish curated query templates — named, parameterized
`StructuredQuery` skeletons — that agents invoke by name with typed parameters
(`list_query_templates`/`run_query_template` over MCP, or the `query-templates`
endpoints over REST), filtered to templates whose connection is visible to the
caller. A bound template runs through the same policy, schema, and guardrail
checks as an ad-hoc query and never accepts raw SQL.
