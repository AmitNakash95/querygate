...
| **Write compensation / undo store** (item 93) | **Shared & correct** when `CONCURRENCY_BACKEND=redis` — the `RedisCompensationStore` (item 93 phase 3b) puts each pre-image in the shared Redis, so `POST /write/undo` resolves a `compensation_id` on **any** replica. With the **in-process** default the store is per-pod, so undo only works on the pod that served the write. Opt-in (`compensation_enabled`); off by default. | Set `CONCURRENCY_BACKEND=redis` (the same setting that shares the concurrency cap + quota) whenever `replicaCount > 1` and reversibility is enabled — the HA overlay does this. Secure that Redis as sensitive: the pre-image holds real row values (TTL bounds exposure; field-level encryption-at-rest is a further hardening). Only the in-process fallback (single-replica) is per-pod. |
