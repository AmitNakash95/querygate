...
| **Write compensation / undo store** (item 93 phase 3a) | **Per-pod, in-process.** A `compensation_id` is captured in the pod that served the write; `POST /write/undo` only resolves it on **that** pod. Under >1 replica, undo of a write served by another pod fails as an unknown id. Opt-in (`compensation_enabled`); off by default. | If you enable reversibility on a multi-replica fleet, route undo to the origin pod (session affinity), or keep it single-replica. A durable cross-replica compensation store (which must also protect the pre-image values at rest) is phase 3b. Until then, treat undo as best-effort under HA. |
