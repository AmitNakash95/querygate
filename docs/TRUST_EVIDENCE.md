...
**0 allowlisted known vulnerabilities.** Every disclosed CVE against the exact shipped dependency set (`poetry.lock`'s `main` group) is fixed by version, not waived. See `make sbom` in [Security & reliability posture](#security--reliability-posture).
...
[docs/THREAT_MODEL.md](THREAT_MODEL.md) enumerates 36 threats (QG-01…QG-36),
