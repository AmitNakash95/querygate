...
*Generated 2026-08-23 for QueryGate 0.1.0. Composed, read-only, from the checked-in docs and dependency-audit results below (TODO.md item 147) — this file asserts no claim of its own; every statement here is backed by the cited source doc and, where named, a reproducible `make` command. It does not imply any control, certification, or third-party attestation that isn't explicitly stated in a source doc.*
...
| **Adversarial suite** | Known bypass classes as regressions | Purpose-built pytest suite (`-m security`) | ✅ 651 tests | `make test-security` |
...
(651 tests, `pytest -m security`) encoding specific known bypass classes:
...
vulnerabilities), `make test-security` (651 tests), and `make test-dast`
