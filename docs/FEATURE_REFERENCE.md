# QueryGate feature reference

The full capability tour, moved out of `README.md` so the README can stay
readable in ten minutes. Nothing here was rewritten — this is the same text,
in the same order, with links repointed for its new location. Start at
[the README](../README.md) if you have not read it yet.

## Contents

- [Persisted audit events](#persisted-audit-events)
- [Example connection config](#example-connection-config)
- [Example policy config](#example-policy-config)
- [Config-governance API (staged versions, apply, rollback)](#config-governance-api-staged-versions-apply-rollback)
- [Example schema catalog (optional)](#example-schema-catalog-optional)
- [Example StructuredQuery payload](#example-structuredquery-payload)
- [Example MCP usage](#example-mcp-usage)
- [Curated query templates (optional)](#curated-query-templates-optional)
- [Agent-visible capacity waiting](#agent-visible-capacity-waiting)
- [Production deployment](#production-deployment)
- [Built-in product guide](#built-in-product-guide)
...
lost. `AUDIT_WORM_SEARCH_MAX_QUERY_SHAPE_DEPTH` (default 64, ceiling 256) bounds how
deep the returned events' `query_shape` is screened for forbidden content
before a line is rejected outright; it fails closed. Optionally set
`AUDIT_WORM_S3_ENDPOINT_URL` to run the tier against an
...
even if it's never selected. See [docs/INFERENCE_RISKS.md](INFERENCE_RISKS.md)
