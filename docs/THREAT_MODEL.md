...
- **Audit durability:** the plain JSONL sink is not WORM storage, has no
  ...
  AWS account root. It does not close the whole residual: WORM archival is
  buffered and asynchronously flushed (fail-open — a sustained S3 outage can
  still lose the oldest buffered events past `AUDIT_WORM_MAX_BUFFERED_EVENTS`,
  visibly, via `querygate_audit_worm_buffer_dropped_total`), and managed
  search over the archive is not built (phase 2, deferred). It complements,
  not replaces, also shipping events to a SIEM.
